...
def main():
    ...
    parser.add_argument('-d', dest="data_fname", default="training", help='File name of the training data.')
    parser.add_argument('-e', dest="eval_data", default="testing", help='Choose whether to evaulate on `testing` or `development` data.')
    parser.add_argument('-t', dest="classifier_name", default=None, help='Choose whether to evaulate on `testing` or `development` data.')
    
    args = parser.parse_args()
    eval_data = "imdb-" + args.eval_data + ".data"
    train_data = "imdb-" + args.data_fname + ".data"

    if args.classifier_name == None:
        for feat_set in ["word_features", "word_pos_features", "word_pos_liwc_features", "word_pos_opinion_features"]:
            ...
            train_eval(train_data, feat_set, ("imdb-" + feat_set + "-model-P1.pickle"), eval_file=eval_data)
    else:
        for feat_set in ["word_features", "word_pos_features", "word_pos_liwc_features", "word_pos_opinion_features"]:
            print("\nTraining with {}".format(feat_set))
            train_eval(train_data, feat_set, args.classifier_name, eval_file=eval_data)
