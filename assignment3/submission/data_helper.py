...
def get_score(review):
    ...
    """
    ###     YOUR CODE GOES HERE
    score = re.search(r"(?<=Overall\s=\s)[0-9]", review).group()
    return score
...
def get_text(review):
    ...
    ###     YOUR CODE GOES HERE
    text = re.search(r"(?<=Text\s=\s\").+(?=\")", review).group()
    return text
...
def get_reviews(raw_data):
    ...
    for review in re.split(r'\.\n', raw_data):
        ...
        if overall_score is not None and int(overall_score) >= 3:
            positive_texts.append(review_text)
        elif overall_score is not None and int(overall_score) < 3:
